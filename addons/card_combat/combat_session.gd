...
func setup(hero: Combatant, hero_cards: Array[CardData], enemy_combatant: Combatant, enemy_cards: Array[CardData], ai_seed: int = -1) -> void:
	...
	player_deck.setup(hero_cards, 0, config.starting_max_mana, ability_fn)
	player_deck.draw_initial_hand(config.initial_hand_size)

	enemy_deck = CombatDeck.new()
	enemy_deck.setup(enemy_cards, 1, config.starting_max_mana, ability_fn)
...
func play_card(card: CardData, as_hidden: bool = false, declared_attack: int = 0, declared_health: int = 0) -> bool:
	...
	if _combat_over:
		return false
	if not player_deck.can_play_card(card):
		return false
	if card.card_type == CardData.CardType.HECHIZO:
		var played: CardData = player_deck.play_spell(card)
		if played == null:
			return false
		_apply_spell_effects(card)
		return true
...
func play_spell(card: CardData, effect: SpellEffect, target: Variant = null) -> bool:
	...
	var context: Dictionary = {"session": self}
...
func _run_ai_turn() -> void:
	...
	while card_to_play != null and plays_this_turn < 10:
		if card_to_play.card_type == CardData.CardType.HECHIZO:
			...
			_apply_enemy_spell_effects(card_to_play)
		else:
...
func _apply_spell_effects(card: CardData) -> void:
	for effect in card.spell_effects:
		_apply_single_spell_effect(effect)


func _apply_single_spell_effect(effect: SpellEffect) -> void:
	match effect.target_type:
		SpellEffect.TargetType.ENEMY_HERO:
			enemy.take_damage(effect.value)
			enemy_damaged.emit(effect.value)
		SpellEffect.TargetType.PLAYER_HERO:
			player_hero.heal(effect.value)
		SpellEffect.TargetType.PLAYER_CREATURE:
			var board: Array[CardInstance] = player_deck.get_board()
			if not board.is_empty():
				effect.apply(board[0], {})
		SpellEffect.TargetType.ENEMY_CREATURES:
			var enemies: Array[CardInstance] = enemy_deck.get_board()
			effect.apply(enemies, {})
			_check_enemy_deaths()
		SpellEffect.TargetType.PLAYER_CREATURES:
			var allies: Array[CardInstance] = player_deck.get_board()
			effect.apply(allies, {})
		SpellEffect.TargetType.SUMMON_BOARD:
			...
			for inst in summoned:
				player_deck.add_to_board(inst)


func _check_enemy_deaths() -> void:
	var board: Array[CardInstance] = enemy_deck.get_board()
	var dead: Array[CardInstance] = []
	for inst in board:
		if inst.is_dead:
			dead.append(inst)
	for inst in dead:
		enemy_deck.remove_from_board(inst)
		creature_died.emit(inst, 1)


func _apply_enemy_spell_effects(card: CardData) -> void:
	for effect in card.spell_effects:
		_apply_single_enemy_spell_effect(effect)


func _apply_single_enemy_spell_effect(effect: SpellEffect) -> void:
	match effect.target_type:
		SpellEffect.TargetType.ENEMY_HERO:
			player_hero.take_damage(effect.value)
			hero_damaged.emit(effect.value)
		SpellEffect.TargetType.PLAYER_HERO:
			player_hero.take_damage(effect.value)
			hero_damaged.emit(effect.value)
		SpellEffect.TargetType.PLAYER_CREATURE, SpellEffect.TargetType.ENEMY_CREATURES:
			var targets: Array[CardInstance] = player_deck.get_board()
			effect.apply(targets, {})
			_check_player_deaths()
		SpellEffect.TargetType.PLAYER_CREATURES:
			var allies: Array[CardInstance] = enemy_deck.get_board()
			effect.apply(allies, {})
		SpellEffect.TargetType.SUMMON_BOARD:
			var result: Dictionary = effect.apply(null, {})
			var summoned: Array = result.get("summoned", [])
			for inst in summoned:
				enemy_deck.add_to_board(inst)


func _check_player_deaths() -> void:
	var board: Array[CardInstance] = player_deck.get_board()
	var dead: Array[CardInstance] = []
	for inst in board:
		if inst.is_dead:
			dead.append(inst)
	for inst in dead:
		player_deck.remove_from_board(inst)
		creature_died.emit(inst, 0)
